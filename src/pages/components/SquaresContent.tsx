...
const withUpdateForm=<div className="squares bg-white text-black my-4 max-w-xs rounded-lg border-gray-500 border-1 shadow-2xl m-0"   >
    ...
    <InfoForm reload={minReloadHandler} parentFormFunction={parentForm} minCash={minCash}/>
    </form>
...
const withoutUpdateForm = <div onClick={updateHandler} className="squares bg-white text-black my-4 max-w-xs rounded-lg border-gray-500 border-1 shadow-2xl m-0"  >
            <div className="square-content flex flex-col mx-5 my-2" >
            <h2 className="text-black">Name: <u>{name}</u></h2>
            <h3>Current Balance: <span className={CashTotal<=minCash ? "text-red-500":"text-green-500"}>{CashTotal}</span></h3>
            <h3>Balance Update: <span style={{color:"green"}}>{balanceAsOf} </span></h3>
  ...
  return(
    ...
    { update ? withUpdateForm:withoutUpdateForm }
